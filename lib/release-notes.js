...
module.exports = class ReleaseNotes {
   ...
   * @return {string}
   */
  static getReleaseType (context, releaseNotesOptions, reuse = true) {
    let rtype = null
    if (!context.nextRelease || !context.nextRelease.type) {
      ReleaseNotes._instance = new ReleaseNotes(context, releaseNotesOptions)
      rtype = ReleaseNotes._instance.getReleaseType()
    }

    if (!rtype) return context.nextRelease.type
  }

  static getReleaseNotes (context, releaseNotesOptions) {
    if (!context.nextRelease || !context.nextRelease.notes) {
      ReleaseNotes._instance = new ReleaseNotes(context, releaseNotesOptions)
    }
